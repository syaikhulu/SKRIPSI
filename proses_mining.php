...
function pembentukan_tree($db_object, $N_parent, $kasus) {
    ...
    if ($cek == 'homogen') {
        ...
        // if($jumlah<=3){
        //     echo "<br>LEAF ";
        //     $Nmenerima = $kondisi." AND kelas_asli='baik'";
        //     $Ntidak = $kondisi." AND kelas_asli='kurang'";
        //     $jumlahmenerima = jumlah_data("$Nmenerima");
        //     $jumlahtidak = jumlah_data("$Ntidak");
        //     if($jumlahmenerima <= $jumlahtidak){
        //         $keputusan = 'kurang';
        //     }else{
        //         $keputusan = 'baik';
        //     }
        ...
        $jml_menerima = jumlah_data($db_object, "$kondisi_kelas_asli kelas_asli='menerima'");
        $jml_tidak = jumlah_data($db_object, "$kondisi_kelas_asli kelas_asli='tidak'");
        
        $jml_total = $jml_menerima + $jml_tidak ;
        echo "Jumlah data = " . $jml_total . "<br>";
        echo "Jumlah menerima = " . $jml_menerima . "<br>";
        echo "Jumlah tidak = " . $jml_tidak . "<br>";

        //hitung entropy semua
        $entropy_all = hitung_entropy($jml_menerima, $jml_tidak);
        ...
        if ($max_gain == 0) {
            echo "<br>LEAF ";
            $Nmenerima = $kondisi . " AND kelas_asli='menerima'";
            $Ntidak = $kondisi . " AND kelas_asli='tidak'";
            $jumlahmenerima = jumlah_data($db_object, "$Nmenerima");
...
function hitung_gain($db_object, $kasus, $atribut, $ent_all, $kondisi1, $kondisi2, $kondisi3, $kondisi4, $kondisi5) {
    ...
    if ($kondisi3 == '') {
        $j_menerima1 = jumlah_data($db_object, "$data_kasus kelas_asli='menerima' AND $kondisi1");
        $j_tidak1 = jumlah_data($db_object, "$data_kasus kelas_asli='tidak' AND $kondisi1");
        $jml1 = $j_menerima1 + $j_tidak1;
        
        $j_menerima2 = jumlah_data($db_object, "$data_kasus kelas_asli='menerima' AND $kondisi2");
        $j_tidak2 = jumlah_data($db_object, "$data_kasus kelas_asli='tidak' AND $kondisi2");
        $jml2 = $j_menerima2 + $j_tidak2 ;
        //hitung entropy masing-masing kondisi
        $jml_total = $jml1 + $jml2;
        $ent1 = hitung_entropy($j_menerima1, $j_tidak1);
        $ent2 = hitung_entropy($j_menerima2, $j_tidak2);
     ...
     //untuk atribut 3 nilai atribut
     	...
     	$j_menerima3 = jumlah_data($db_object, "$data_kasus kelas_asli='menerima' AND $kondisi3");
     	$j_tidak3 = jumlah_data($db_object, "$data_kasus kelas_asli='tidak' AND $kondisi3");
     	$jml3 = $j_menerima3 + $j_tidak3;
        
     	//hitung entropy masing-masing kondisi
     	$jml_total = $jml1 + $jml2 + $jml3;
     	$ent1 = hitung_entropy($j_menerima1 , $j_tidak1);
     	$ent2 = hitung_entropy($j_menerima2 , $j_tidak2);
     	$ent3 = hitung_entropy($j_menerima3 , $j_tidak3);
     	$gain = $ent_all - ((($jml1/$jml_total)*$ent1) + (($jml2/$jml_total)*$ent2) 
     				+ (($jml3/$jml_total)*$ent3));							
     	//desimal 3 angka dibelakang koma
     	$gain = format_decimal($gain);				
